...
CodexVisual is an open-source Codex quota tracker for the macOS menu bar and Windows taskbar tray. It shows your remaining Codex 5-hour and 7-day usage quota from local Codex logs, without reading auth tokens or calling an external quota service.

Useful search terms: Codex quota tracker, Codex usage limit, Codex 5-hour limit, Codex 7-day limit, Codex menu bar app, Codex Windows tray app, OpenAI Codex quota monitor.

## English

CodexVisual is a lightweight macOS menu bar and Windows tray app for checking your remaining Codex quota at a glance.

It focuses on one thing: showing the remaining 5-hour quota and 7-day quota where you can see it without opening logs.
...
| Windows | 1.0.13 | [GitHub Releases](https://github.com/orangeshushu/CodexVisual/releases) |
...
CodexVisual is intentionally small and focused. It is for people who search for a simple Codex quota monitor, Codex usage tracker, Codex menu bar app, or Windows tray quota widget. Compared with [steipete/CodexBar](https://github.com/steipete/CodexBar), CodexVisual is lighter and only targets Codex quota visibility.
...
Open [GitHub Releases](https://github.com/orangeshushu/CodexVisual/releases) and download `CodexVisual-Windows.exe` from the newest release that includes Windows assets. The Windows executable is built by the Windows release workflow, so avoid using `/releases/latest/download/CodexVisual-Windows.exe` unless the current latest release contains that asset.
...
CodexVisual 是一个开源的 Codex 额度查看工具，支持 macOS 菜单栏和 Windows 任务栏托盘。它从本地 Codex 日志中读取 5 小时额度和 7 天额度，不读取 auth token，也不调用外部额度服务。

它只专注一件事：在你最容易看到的位置显示 Codex 的 5 小时额度和 7 天额度剩余百分比。

常见搜索词：Codex 额度查看、Codex 剩余额度、Codex 使用限制、Codex 5 小时限制、Codex 7 天限制、Codex 菜单栏工具、Codex Windows 托盘工具。
...
CodexVisual 是一个更轻量、更单一用途的额度显示工具，适合正在搜索 Codex 额度查看、Codex 使用量监控、Codex 菜单栏工具或 Windows 托盘额度窗口的用户。相比 [steipete/CodexBar](https://github.com/steipete/CodexBar)，CodexVisual 只针对 Codex 的本地额度状态展示，不做额外的工作流管理。

### 推广和搜索优化

- [搜索优化说明](docs/SEO.md)：包含推荐 GitHub 描述、topics 和关键词。
- [推广文案](docs/PROMOTION.md)：包含可以直接发布的中英文介绍。
...
打开 [GitHub Releases](https://github.com/orangeshushu/CodexVisual/releases)，在包含 Windows 资产的最新 release 中下载 `CodexVisual-Windows.exe`。Windows 可执行文件由 Windows release workflow 构建并上传；除非当前 latest release 确实包含该资产，否则不要使用 `/releases/latest/download/CodexVisual-Windows.exe`。
